...
def prepare_bert_input():
    ...
    for cur_pid in tqdm(pids_train):
        ...
        for bib in cur_pos_bib:
            ...
            #if len(bib_to_contexts[bib])>=1:
            #    cur_context = bid_to_title[bib]+bib_to_contexts[bib][0]
            #else:
            #    cur_context = bid_to_title[bib]+""
            cur_context = bid_to_title[bib]+" ".join(bib_to_contexts[bib])
            #print(len(cur_context))
...
def train(year=2023, model_name="scibert"):
    ...
    OUTPUT_DIR = join(settings.OUT_DIR, "kddcup", model_name+'_eval1-429')
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    log_file = open('./out/kddcup/scibert_eval1-429/'+str(time.time())+'.txt','w')
...
def eval_test_papers_bert(model):
   ...
   # papers_dblp = utils.load_json(data_year_dir, "paper_info_hit_from_dblp.json")
    ...
    for paper in tqdm(papers_test):
        ...
        #+bid_to_title[bib]
        #abstract = bs.find_all("profileDesc")[0].abstract.text.lower()  " ".join(bib_to_contexts[bib])
        #contexts_sorted=[]
        #for bib in bib_sorted:
...
def gen_kddcup_valid_submission_bert(model_name="scibert"):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(BERT_MODEL, num_labels = 2)
    model.load_state_dict(torch.load(join(settings.OUT_DIR, "kddcup", model_name+'_eval1-429', "pytorch_model.bin")))
    model.to(device)
    model.eval()
    ...
    utils.dump_json(sub_dict, join(settings.OUT_DIR, "kddcup", model_name+'_eval1-429'), "test_submission_scibert.json")
