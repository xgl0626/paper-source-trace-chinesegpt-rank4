...
import utils
import settings
#from oag.bert_model import BertConfig, BertModel
#from oag.oagbert import OAGBertPretrainingModel
...
def prepare_bert_input():
    ...
    papers = utils.load_json(data_dir, "paper_source_trace_train.json")
...
def train(year=2023, model_name="scibert"):
    ...
    #with open(join(data_year_dir, "bib_context_train1-0.449.txt"), "r", encoding="utf-8") as f:
    #    for line in f:
    #        train_texts.append(line.strip())

    #with open(join(data_year_dir, "bib_context_train_label1-0.449.txt"), "r", encoding="utf-8") as f:
    #    for line in f:
    #        train_labels.append(int(line.strip()))

    with open(join(data_year_dir, "bib_context_train1_data.txt"), "r", encoding="utf-8") as f:
        for line in f:
            train_texts.append(line.strip())

    with open(join(data_year_dir, "bib_context_train_label1_data.txt"), "r", encoding="utf-8") as f:
        for line in f:
            train_labels.append(int(line.strip()))
    ...
    #config = BertConfig.from_dict(json.load(open(os.path.join(BERT_MODEL, "config.json"))))
    #model = OAGBertPretrainingModel(config)
    #model_para=torch.load('./saved/oagbert-v2-sim/pytorch_model.bin')
    ...
    #for k,v in model_para.items():
        #print(k)
        #PreAttentionLayerNorm attention.output.LayerNorm
        #PostAttentionLayerNorm output.LayerNorm
        #if 'dense_act' in k:
        #    k = k.replace('dense_act','dense')
        #if 'PreAttentionLayerNorm' in k:
        #    k = k.replace('PreAttentionLayerNorm','attention.output.LayerNorm')
        #if 'PostAttentionLayerNorm' in k:
        #    k = k.replace('PostAttentionLayerNorm','output.LayerNorm')
            
        #name = k.split('.')
        #name = '.'.join(name[1:])
        #new_parameter_dict[k] = v    
    #config = BertConfig.from_pretrained('./saved/oagbert-v2-sim',num_labels = 2)
    #model = BertForSequenceClassification(config= config)
    #for k,v in model.named_parameters():
        #print(k)     
    #model.load_state_dict(new_parameter_dict,strict=False)
    ...
    #  "weight_decay": 0.0, "lr": CLS_LEARNING_RATE
    #  }
    #]
    ...
    OUTPUT_DIR = join(settings.OUT_DIR, "kddcup", 'pretrain')
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    log_file = open('./out/kddcup/pretrain/'+str(time.time())+'.txt','w')
...
def eval_test_papers_bert(model):
    data_year_dir = join(settings.DATA_TRACE_DIR, "PST")
    papers_test = utils.load_json(data_year_dir, "paper_source_trace_dev.json")
