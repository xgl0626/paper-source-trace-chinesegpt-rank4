...
def train(year=2023, model_name="scibert"):
    ...
    OUTPUT_DIR = join(settings.OUT_DIR, "kddcup", model_name+'_eval1-423')
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    log_file = open('./out/kddcup/scibert_eval1-423/'+str(time.time())+'.txt','w')
...
def gen_kddcup_valid_submission_bert(model_name="scibert"):
    ...
    model = BertForSequenceClassification.from_pretrained(BERT_MODEL, num_labels = 2)
    model.load_state_dict(torch.load(join(settings.OUT_DIR, "kddcup", model_name+'_eval1-423', "pytorch_model.bin")))
    model.to(device)
    model.eval()
    ...
    utils.dump_json(sub_dict, join(settings.OUT_DIR, "kddcup", model_name+'_eval1-423'), "test_submission_scibert.json")
